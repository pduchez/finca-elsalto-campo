...
const CAB = ["Colaborador", "Días trabajados (de 12)", "Jornal", "Destajo", "Total a pagar"];
...
function aoa(pl: Planilla): (string | number)[][] {
  return [
    ...
    [],
    CAB,
    ...pl.filas.map((f) => [f.nombre, f.diasTrabajados, f.pagoJornal, f.pagoDestajo, f.totalPago]),
    ["TOTALES", pl.totalDias, pl.totalJornal, pl.totalDestajo, pl.totalPago],
  ];
}
...
export async function exportarPlanillaExcel(pl: Planilla): Promise<void> {
  ...
  ws["!cols"] = [{ wch: 26 }, { wch: 20 }, { wch: 12 }, { wch: 12 }, { wch: 14 }];
