...
- **Login con usuario y contraseña**. Cuatro usuarios iniciales (Director1,
  Director2, Supervisor1 = Emerson, Administrativo1), contraseña inicial
  `password`, y el sistema **exige cambiarla en el primer ingreso**. La sesión
  es **permanente** (no caduca): quien entra queda firmado.
...
- **Fotos y audio** suben a **Supabase Storage** (bucket `capturas`) en cuanto
  hay señal, después de que sube la fila. Idempotente (no duplica).
...
- **Panel del dueño**: hoy lee los datos locales del dispositivo; falta que lea
  de Supabase (y muestre las fotos vía URLs firmadas) para ver todo desde otra
  máquina.
- **Audio → IA (Claude)**: falta un proveedor de _speech-to-text_ que transcriba
  el audio antes de la extracción.
- **Gestión de usuarios en pantalla**: hoy los usuarios se crean/editan en la
  tabla `usuarios` de Supabase; falta una pantalla de administración.
...
2. Menú izquierdo → **SQL Editor** → **New query**. Pegá TODO el contenido de
   ...
   - **Si ya habías corrido el schema antes** (base existente): corré una sola
     vez `supabase/migracion-usuarios-storage.sql`. Agrega la tabla `usuarios`,
     los 4 usuarios iniciales y la columna de fotos del área. Es idempotente.
3. Menú izquierdo → **Storage**: verificá que exista el bucket **`capturas`**
   (lo crea el schema). Si no está, crealo (privado). Ahí caen el audio y las
   fotos, en carpetas `registros/`, `asistencia/` y `areas_detalle/`.
...
3. Abrí **Environment Variables** y agregá (Environment: **Production** y
   **Preview**):
   ...
   | `AUTH_SECRET` | cadena larga y aleatoria | Recomendada (si no, usa la Secret) |
   | `ANTHROPIC_API_KEY` | de console.anthropic.com | Opcional (IA de audio) |

   > `AUTH_SECRET` firma la cookie de sesión. Si la dejás vacía, el sistema usa
   > `SUPABASE_SERVICE_ROLE_KEY` (que ya es secreta). Poné una propia si querés
   > poder rotar la clave de la base sin cerrar las sesiones.
...
3. Queda un ícono en la pantalla de inicio llamado **Finca El Salto**. Al tocarlo,
   la app abre a pantalla completa (sin barra del navegador).
4. La primera vez, con señal, dejá que cargue: se guardan la app, el plan y los
   protocolos para usarse sin señal.
5. Aceptá los permisos de **cámara/micrófono** y **ubicación** una vez. El GPS
   funciona sin datos.

## Paso 5 — Usuarios y contraseñas

- Usuarios iniciales: **Director1**, **Director2**, **Supervisor1** (Emerson),
  **Administrativo1**. Contraseña inicial de todos: **`password`**.
- En el **primer ingreso** el sistema obliga a cambiar la contraseña.
- La sesión **no caduca**: una vez dentro, el usuario queda firmado (útil para
  Emerson en el campo, sin tener que reingresar).
- Directores y Administrativo entran al **panel**; el Supervisor entra a la
  **app de campo**.
- Para agregar o desactivar usuarios (mientras no haya pantalla), editá la tabla
  `usuarios` en Supabase. Para resetear una contraseña, poné
  `debe_cambiar_password = true` y volvé a fijar el hash inicial (`password`)
  desde `supabase/seed.sql`.

## Paso 6 — Probar que quedó vivo

1. Ingresá con un usuario, cambiá la contraseña.
2. Registrá una asistencia con foto y una tarea a destajo.
3. En Supabase → **Table Editor** → tabla `asistencia` / `tareas_destajo`:
   deben aparecer las filas. En **Storage → capturas** deben aparecer las fotos.
4. Prueba offline: activá **modo avión**, hacé un registro (verás “X
